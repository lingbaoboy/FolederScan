...
class DirectoryScannerApp:
            ...
            for item in current_path.iterdir():
                try:
                    ...
                    if self._should_log_item(item, exclude_set, exclude_case_sensitive, fname_keyword_set,
                        ...
                        line = f"{item.name}@{relative_path}@{mod_time}@{create_time}\n"
                        output_file.write(line)

                    # 决策 2: 判断是否应深入遍历此目录 (此决策与是否写入日志无关)
                    if item.is_dir():
                        # 唯一的遍历停止条件是“目录停止关键字”
                        ...
                        if stop_keyword_set and any(keyword in name_to_check for keyword in stop_keyword_set):
                            continue  # 命中停止关键字，跳过对此目录的深入，继续检查同级的其他项目
    ...
    def run_scan(self, start_path_str, max_depth, stop_keyword_set, stop_case_sensitive, exclude_set,
        ...
        try:
            ...
            output_file_path = start_path / "scan_results.txt"
            with open(output_file_path, "w", encoding="utf-8") as output_file:
                output_file.write("文件名@相对路径@修改时间@创建时间\n")
                self._recursive_scan(start_path, start_path, 0, max_depth, stop_keyword_set, stop_case_sensitive,
                                     exclude_set, exclude_case_sensitive, fname_keyword_set, fname_case_sensitive,
                                     fname_mode, output_file)
